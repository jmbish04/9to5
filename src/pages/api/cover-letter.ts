...
export async function POST({ locals, request }: { locals: any; request: Request }) {
  ...
  try {
    ...
    const agentRequest = AgentOrchestrator.createCoverLetterRequest(
      user_profile.user_id || 'anonymous',
      job_id,
      user_profile,
      { tone, length, highlights }
    );
    ...
    return Response.json(
      {
        ...
        cover_letter: response.data,
        metadata: response.metadata
      },
      { headers: { 'Access-Control-Allow-Origin': '*' } }
    );
  } catch (err) {
    console.error('POST /api/cover-letter failed', err);
    return Response.json(
      { error: { code: 'internal_error', message: 'Failed to generate cover letter' } },
      { status: 500, headers: { 'Access-Control-Allow-Origin': '*' } }
    );
  }
}